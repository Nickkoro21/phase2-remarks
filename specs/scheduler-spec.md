...
## 11. INSTRUCTOR CURRENCY — **ΔΙΚΗ ΤΗΣ ΚΑΡΤΕΛΑ** (Γύρος 11, 18/08/2026)
...
**Ταυτότητα κάλυψης:** πίνακας ① (15) + πίνακας ② (76) = **91**, κάθε id ακριβώς
μία φορά. Ελέγχεται στο boot (`curCoverage()` → `console.warn` αν χαλάσει).

### 11α. ΔΙΑΤΑΞΗ

| | |
|---|---|
| **αριστερά** `#cur-list` | Η λίστα εκπαιδευτών από `SchedStore.instructors` — ενεργοί πρώτοι, μετά φυσική σειρά κωδικού (IP-2 πριν από το IP-10). Κάθε γραμμή: **τελεία διαθεσιμότητας** · code · βαθμός · όνομα · `owes N` (ή `exp N`) · chip **`sem K/M`**. Κλικ επιλέγει· η επιλογή **επιβιώνει** κάθε repaint και κάθε εναλλαγή καρτέλας (ui state του module — δεν γράφεται στο store). |
| **δεξιά** `#cur-main` | Κεφαλίδα του επιλεγμένου (code · βαθμός · όνομα · callsign · χώρα · TP · **pill διαθεσιμότητας** · chip εξαμήνου · διακόπτης **EXPERIENCED (ΕΜΠ)** · **🖨 Print**) και **οι ΔΥΟ πίνακες**. |

Η τελεία, το `owes N` και το pill διαβάζουν **μόνο τον πίνακα ②**. Το `sem K/M`
είναι **ξεχωριστό** άθροισμα: μια ποσόστωση δεν είναι παράθυρο και δεν κοστίζει
διαθεσιμότητα (§ 11β).
...
χειριστήριο currency· το tooltip του ξαναγράφτηκε ώστε να **μην** αναφέρει καρτέλα ή
κάρτα. Ο διακόπτης που το αλλάζει ζει πλέον **μόνο** στην καρτέλα Currency.
