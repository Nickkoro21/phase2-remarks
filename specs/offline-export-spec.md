...
> VERIFY: ό,τι δεν σάρωνε ποτέ το patch· §§ 1, 3γ, 3η, 3θ, 5)** · **Αναθεώρηση:
> 2026-08-28 (Γύρος 24b — Η ΑΝΤΙΣΤΑΘΜΙΣΗ ΤΑΞΙΔΕΥΕΙ ΜΕ ΤΟ ΣΧΗΜΑ: το ένα υπόλοιπο
> του verify· §§ 3γ.6, 3η)**
...
- **3γ. `gap`:** κάθε `gap`/`row-gap`/`column-gap` παίρνει το legacy
  ...
  **Γύρος 24 — ο κανόνας του φύλλου, όπως τον έφτιαξε το verify:**
  ...
  6. **Η ΑΝΤΙΣΤΑΘΜΙΣΗ ΤΑΞΙΔΕΥΕΙ ΜΕ ΤΟ ΣΧΗΜΑ (Γύρος 24b — το ένα υπόλοιπο του
     verify).** Ο κανόνας 5 έμαθε στις γραμμές των **παιδιών** να ακολουθούν το
     flip μέσα στο @media· οι γραμμές του **container** έμεναν εκεί που
     γράφτηκαν. Η `.fc-bar` κάνει wrap και δεν ζωγραφίζει τίποτα, άρα πήρε
     `margin-bottom:-14px` στο top level· στα 740px το stylesheet τη γυρίζει σε
     column, τα παιδιά της μηδενίζονται σωστά — και τα -14px συνέχιζαν να
     ισχύουν αντισταθμίζοντας **το τίποτα**. Μετρημένο στα 560px με το fallback
     αναγκασμένο: τα 12px που βάζει το ίδιο το gap της `.fc-view` ανάμεσα σε
     `.fc-bar` και `.fc-l0` έβγαιναν **−2px, δηλαδή επικάλυψη**. Πλέον η
     ερώτηση γίνεται σε **κάθε** σχήμα και διαβάζεται από τη γραμμή των παιδιών
     (μόνο η ουρά `> *` του wrap βάζει margin **και** στο τελευταίο παιδί, άρα
     μόνο αυτή μεγαλώνει το κουτί), και **κάθε** context όπου βγαίνει γραμμή
     container ξαναδηλώνει **ΟΛΕΣ** τις ιδιότητες που έχει βάλει ποτέ ο
     generator σε αυτό το κουτί — με την τιμή του νέου σχήματος ή, όταν το νέο
     σχήμα δεν χρειάζεται τίποτα, με τη δική του ανεπηρέαστη τιμή. Προστέθηκε
     **μία** γραμμή σε όλο το export
     (`@media (max-width: 740px){html.no-flexgap .fc-bar{margin-bottom:0}}`).
  ...
  **Γύρος 24b, ίδια διαδρομή στα 560px:** `.fc-bar` → `.fc-l0` **12px**
  (baseline του gap), computed `margin-bottom: 0px`, καμία επικάλυψη· με τα
  -14px επαναφερμένα χειροκίνητα ξαναβγαίνει −2px, άρα η γραμμή που προστέθηκε
  είναι ακριβώς αυτή που μετράει. Και οι τέσσερις **δηλωμένοι** ψηλότεροι
  containers (`.fc-vrow-chain` +1px, `.sch-actrow` +2px, `.sch-pnode` +5px,
  `.sch-pcat` +5px) μένουν δηλωμένοι και **byte-identical** στο φύλλο.
...
- **3η. CSS-IN-JS — ΜΙΑ ΛΙΣΤΑ, ΟΧΙ ΔΥΟ (Γύρος 24).** Τέσσερα blocks γράφουν CSS
  ...
  Fx32-65 το backdrop του Schedule Validation και της κάρτας minimums μάζευε
  πάνω-αριστερά. **Μετρημένο στο ίδιο το αρχείο: με `inset` καλύπτει ΟΛΟ το
  viewport (1440×900)· χωρίς αυτό γίνεται ένα κουτί στη γωνία πάνω-αριστερά με
  μέγεθος περιεχομένου — όχι κάποια σταθερή διάσταση: το verify το μέτρησε
  760×305 και 1240×115 στα δύο modals.**
