# Description — προδιαγραφή (Γύροι 16 · 16b)
...
| **F3203** | F3201-3 | F3101-2 = 0 | 1 | **1** | **τελευταία πτήση της κατηγορίας** — πιάνει το τέλος ενότητας (0 → 1) |
| N2304 | N2301-04 | N2201-02 = 2 | 3 | **3** | τελευταία πτήση της κατηγορίας |
| C5490 · I4890 · F4690 · N4690 · C2604 · I3601 | — | 3 | 3 | **3** | τελευταίες πτήσεις — η ράμπα έχει ήδη πιάσει το 3 |

Η **F3203** είναι η μοναδική έξοδος όπου ο δεύτερος κανόνας φαίνεται με γυμνό
μάτι (0 → 1)· παντού αλλού η προηγούμενη ενότητα έχει ήδη φτάσει στο τελικό MIF.
...
| **η πτήση** | τα άδεια κελιά **αυτής** της στήλης βγαίνουν γκρίζα («not planned in this sortie»)· όσα απαγορεύονται δια βίου για solo βγαίνουν διαγραμμένα (§14b(6))· όσα έχουν **τρέχον** MIF κάτω από τον Κώδικα «2» βγαίνουν με διακεκομμένο περίγραμμα (3-01 §28θ). Το tooltip κάθε αριθμού γράφει το όνομα και το τρέχον MIF. |
...
Η σπορά (MIF ≥ 2) και η σύμπτυξη σε εύρη (`#1-2, #5, #22-24`) μένουν ως έχουν.
